...
function Ver_pdf($contenido, $nombre_archivo) {
    ...
    $body = $contenido;
//    $body = obtenerBody(cambiarVerLogo($contenido,$nit));
    $stylesheet = file_get_contents('facturaa.css');
...
function GenerarPdf_Server($contenido, $nombre_archivo, $nit) {
    $mpdf = new mPDF();
    $mpdf->charset_in = 'UTF-8';
    $body = obtenerBody(cambiarVerLogo($contenido,$nit));
    ...
    $ext = strtoupper(substr($nombre_archivo, -4));
    $nombre = ($ext == ".PDF") ? $nombre_archivo : $nombre_archivo.='.pdf';
    $mpdf->Output($nombre, 'F'); //Guarda el archivo en el servidor
}
...
function cambiarVerLogo($html,$nit) {
    $srcLogo = '../../clauserp/logos/'.$nit.'.jpg';
    return str_replace('/cgi-bin/clauspro/VerLogo.php',  $srcLogo, $html);
}
